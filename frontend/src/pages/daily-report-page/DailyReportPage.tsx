...
import { DateCalendar, SectionTitle } from '@/components/shared';
  ...
  return (
    <div className="mt-32.5 flex flex-col gap-5 pb-29.5">
      <header className="flex justify-between">
        <SectionTitle
          title="하루 리포트"
          description={`다음 하루 리포트는 ${mockedDate.toLocaleDateString()} ${mockedDate.toLocaleTimeString()}에 발행돼요.`}
        />
            ...
            <div>
              ...
              <div className="flex flex-col gap-3">
                {Array(3)
                  ...
                  .map(() => (
                    <p className="bg-grey-100 rounded-200 text-grey-900 body-small-medium [&>strong]:text-brand-main px-4 py-3">
                      취소건수 증가가 이어지는지 확인하기 위해 취소건수와
                      주문건수를 함께 보고, 취소건수 비중이 커지는 흐름인지
                      점검해봐요.
                    </p>
                  ))}
              </div>
