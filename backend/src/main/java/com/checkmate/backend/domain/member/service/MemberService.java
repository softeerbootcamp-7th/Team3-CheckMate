...
public class MemberService {
    ...
    public void logout(Long memberId) {
        ...
        memberAuthRepository.findByMember(member).ifPresentOrElse(
                memberAuth -> {
                    memberAuth.updateRefreshToken(null);
                    log.info("Member {} logged out successfully. Refresh token invalidated.", memberId);
                },
                () -> log.warn("Member {} has no MemberAuth record. Already logged out.", memberId)
        );
    }
}
